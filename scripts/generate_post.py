...
1. 글 상단에 Jekyll 블로그용 **Front Matter**를 YAML 형식으로 포함해줘. `---`로 감싸고 다음 항목을 채워줘:
   ...
   - `categories`: 글의 주제에 맞는 1개 이상의 카테고리, 2개 이상인 경우 "[]"로 묶어서 배열형태로 작성
   - `tags`: **반드시 `java`, `springboot`, `ai` 중 하나 이상**을 포함하고, 총 2개 이상 7개 이하의 핵심 키워드, 2개 이상인 경우 "[]"로 묶어서 배열형태로 작성
...
4. 각 본문 구조의 제목은 "서론", "주요 개념 설명", "방식 또는 종류별 비교", "각 방식의 장단점 분석", "마크다운 테이블로 정리" 같은 고정 문구 대신, **글 내용에 어울리는 자연스러운 소제목**으로 작성해줘.

5. **검색엔진 최적화(SEO)**를 고려해, 해당 주제와 관련된 검색량이 높은 **관련 키워드, 기술용어, 구체적인 표현**을 본문에 적극적으로 사용해줘. 예: "JPA 성능 개선", "Spring Boot 설정 최적화", "AI 모델 튜닝" 등

6. 마크다운 문법을 정확히 사용해:
   - 제목은 `##`, 소제목은 `###`, 목록은 `-`, 표는 `|` 사용
   - 코드 블록은 언어명 포함해서 사용 (예: ```java)
   - "마크다운 테이블로 정리"에 해당하는 제목 아래 공백 한줄 띄어서 작성
